...
class RA_CORE_API PolyLine {
  ...
  public:
    ...
    Scalar squaredDistance( const Math::Vector3& p ) const;

    /// Return the distance between the line and a given point p.
    Scalar distance( const Math::Vector3& p ) const;

    /// Returns the parameter t in [0,1] corresponding to the point on the
    /// ith segment closest from point p.
    Scalar projectOnSegment( const Math::Vector3& p, uint segment ) const;

    /// Returns the index of the nearest segment.
    uint getNearestSegment( const Math::Vector3& p ) const;
    ...
    Scalar project( const Math::Vector3& p ) const;
