...
void SpotLight::setTransform( Core::Container::Index roIdx, const Core::Math::Transform& transform ){
    ...
    else  if (transform(1,0)==0 && transform(1,1)==1 && transform(1,2)==0){
      //Rotation alongside Y
      m_direction = Core::Math::Vector3(
    ...
    else  if (transform(2,0)==0 && transform(2,1)==0 && transform(2,2)==1){
      //Rotation alongside Z
      m_direction = Core::Math::Vector3(
