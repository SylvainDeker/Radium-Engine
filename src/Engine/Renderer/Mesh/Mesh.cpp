...
Mesh::Mesh( const std::string& name, MeshRenderMode renderMode ) :
    ...
    m_numElements( 0 ),
    m_isDirty( false ) {
    CORE_ASSERT( m_renderMode == RM_LINES || m_renderMode == RM_LINES_ADJACENCY ||
                     m_renderMode == RM_TRIANGLES || m_renderMode == RM_POINTS ||
...
/*        // Template parameter must be a Core::VectorNArray
        ...
        void Mesh::sendGLData( const VecArray& arr, const uint vboIdx )
        {
            ...
            {
                GL_ASSERT( glBindBuffer( GL_ARRAY_BUFFER, m_vbos[vboIdx] ) );
                GL_ASSERT( glBufferData( GL_ARRAY_BUFFER, arr.size() * sizeof( typename
VecArray::Vector ), arr.data(), GL_DYNAMIC_DRAW ) ); m_dataDirty[vboIdx] = false;
            }
        }
*/
void Mesh::updateGL() {
