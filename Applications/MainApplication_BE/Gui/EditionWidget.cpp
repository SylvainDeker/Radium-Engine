...
/**
    ...
    {
        Ra::Core::Math::Transform tf;
        if(getTransform(&tf))
        {
            setEnabled(true);
            Ra::Core::Math::Matrix4 m = tf.matrix();
            Ra::Core::Math::Matrix3 r = tf.rotation();
            Ra::Core::Math::Vector3 angles = r.eulerAngles(0,1,2);

            if(doProperties)
            {
                //let's hope there's no negative scaling
                Ra::Core::Math::Vector3 v(m(0,0), m(1,0), m(2,0));
                SET_VALUE_NO_SIGNALS(m_scale_x, v.norm());
                v = Ra::Core::Math::Vector3(m(0,1), m(1,1), m(2,1));
                SET_VALUE_NO_SIGNALS(m_scale_y, v.norm());
                v = Ra::Core::Math::Vector3(m(0,2), m(1,2), m(2,2));
    ...
    bool EditionWidget::setTransform(const Ra::Core::Math::Transform& tf, bool doUpdateInfos=true)
    {
    ...
    bool EditionWidget::setMatrix(Core::Math::Matrix4 &m)
    {
        Ra::Core::Math::Transform tf(m);
        return setTransform(tf);
    }

    bool EditionWidget::getTransform(Ra::Core::Math::Transform* tf)
    {
        Ra::Engine::ItemEntry item = m_selectionManager->currentItem();
    ...
    void EditionWidget::resetSelectedObject()
    {
        ...
        Ra::Core::Math::Transform id = Ra::Core::Math::Transform::Identity();
        setTransform(id);
    }
       ...
       if (m_use_tranform_matrix->isChecked()){
            ...
            case 0 :
                if(!applyMatrixText()){
                    ...
                    LOG(Core::Utils::logINFO) << "Cannot apply matrix : bad format or non-editable object.";
                }
                break;
            case 1 :
                if(!applyMatrixBox()){
    ...
    bool EditionWidget::transformation(){
        Core::Math::Transform tf = Core::Math::Transform::Identity();
        //scale, then rotation, and finally translation (inverted because matrix multiplication)
        tf.translate(Core::Math::Vector3(m_translation_x->value(), m_translation_y->value(), m_translation_z->value()));
        ...
        tf.scale(Core::Math::Vector3(m_scale_x->value(), m_scale_y->value(), m_scale_z->value()));
    ...
    bool EditionWidget::applyMatrixText()
    {
        ...
        Core::Math::Matrix4 m = Core::Math::Transform::Identity().matrix();
        ...
        std::vector<std::string> toks = Ra::Core::Utils::splitString(input, ',');
        //deal with locale
